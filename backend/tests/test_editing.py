...
def test_reestimate_empty_edit_is_rejected(tmp_path):
    # An empty edit is a client bug, not a no-op: rejected up front so it
    # never writes a spurious restorable version (see test_reestimate.py).
    client, _ = make_client(tmp_path)
    sid = upload(client).json()["session_id"]
    before = client.get(f"/sessions/{sid}").json()["estimate"]
    assert client.post(f"/sessions/{sid}/reestimate", json={}).status_code == 400
    assert client.get(f"/sessions/{sid}").json()["estimate"] == before
